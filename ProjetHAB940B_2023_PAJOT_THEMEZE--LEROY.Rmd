...
ggplot(gopher, aes(x=prev, y=shells,color=Site, shape=year)) +
  ...
  facet_wrap(~Site)+
  labs(caption= "Figure 1 : Le nombre de carapaces en fonction de la prevalence de tortues séropositives",
       x = "prev") +
...
ggplot(gopher, aes(x=Area, y=prev,color=Site)) +
  geom_point() +
  facet_wrap(~Site)+
  labs(caption= "Figure 1 : La prevalence de  en fonction de l'année et du site d'étude",
       x = "Area") +
  theme(plot.caption = element_text(hjust = 0, face = "italic", size = 12))
