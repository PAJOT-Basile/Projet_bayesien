...
L'équation (apporche fréquentiste) du modèle linéaire simple s'écrit de la manière suivante : \newline 
$$\frac {shells} {aire_{site}} = \mu_0 + \beta * prev$$
Ceci se traduit an approche bayesienne par un modèle considérant les hypothèses suivantes : 
...
Nous avons donc :
$$Shells_i \space \stackrel{i.i.d}{\sim} \space Pois(\lambda_i) \space avec \space i=1,..30 \space le \space nombre \space d'observations$$  
$$log(\lambda_i) =  log({aire_{i}}) + \mu_0 + \beta * prev_i$$ 
\newline 
Nous utilisons comme priors les distribution suivantes : \newline 
$$\mu_0 \space {\sim} \space \mathcal{N}(0, 100)$$
$$\beta \space {\sim} \space \mathcal{N}(0, 100)$$
