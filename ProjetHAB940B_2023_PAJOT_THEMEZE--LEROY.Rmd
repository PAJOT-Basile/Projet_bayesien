...
random_model <- function(){
  ...
  for(i in 1:N){
    S[i] ~ dpois(lambda[i])
    log(lambda[i]) <- mu.s + alpha.s[site[i]] + b.prev * prev[i] + alpha.y.0* Cov.y.0[i] + alpha.y.1 * Cov.y.1[i] + alpha.y.2 * Cov.y.2[i] + log(A[i])
  }
  for (j in 1:nb.sites){
...
for (i in 1:nrow(gopher)){
  shells6[,i] <-gopher$Area * exp(res$mu.s[i] + res$alpha.y.0[i] * gopher$Cov.y.0 + res$alpha.y.1[i] * gopher$Cov.y.1 + res$alpha.y.2[i] * gopher$Cov.y.2 + res$b.prev[i] * gopher$standprev)
}
hist(shells6)
mean(shells6)
