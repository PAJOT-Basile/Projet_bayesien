...
simulated_shells <- rep(NA, nrow(res))
for (i in 1:nrow(res)){
  lambda <- gopher$Area * exp(res$mu.0[i] + res$alpha_1[i] * gopher$Cov_2005 + res$alpha_2[i] * gopher$Cov_2006 + res$b.prev[i] * gopher$standprev + rnorm(1, mean=0, sd=res$sd.s[i]))
  simulated_shells[i] <- rpois(1, lambda)
}
hist(simulated_shells)
mean(simulated_shells)
