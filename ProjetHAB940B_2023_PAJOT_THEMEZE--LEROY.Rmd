...
# Evaluons la convergence du modèle
traceplot(M1, mfrow=c(1, 2), ask=FALSE)
par(mfrow = c(1, 1))

# On récupère les paramètres et on regarde leur distribution
res0 <- M1$BUGSoutput$sims.matrix %>% 
  as.data.frame()
hist(res0$mu.0)

# Rétrotransformation
shells0 <- matrix(NA,ncol=nrow(gopher),nrow=nrow(res0))
for (i in 1:nrow(gopher)){
  shells0[,i] <-gopher$Area[i] * exp(res0$mu.0)
}
hist(shells0)
mean(shells0)
...
Un modèle avec uniquement l'année `M3`  : $$log(\lambda_i) =  log({aire_{i}}) + \mu_0 + \alpha_0 * cov_{2004}+\alpha_1 * cov_{2005} +\alpha_2 * cov_{2006}$$
```{r}
...
res3 <- M3$BUGSoutput$sims.matrix %>% 
  as.data.frame()
hist(res3$mu.0)
hist(res3$alpha_0)
hist(res3$alpha_1)
...
# Rétrotransformation
shells3 <- matrix(NA,ncol=nrow(gopher),nrow=nrow(res3))
for (i in 1:nrow(gopher)){
  shells3[,i] <-gopher$Area[i] * exp(res3$mu.0 + res3$alpha_0 * gopher$Cov_2004[i] + res3$alpha_1 * gopher$Cov_2005[i] + res3$alpha_2 * gopher$Cov_2006[i])
}
hist(shells3)
mean(shells3)
...
Un modèle avec l'année et la prévalence (continue) `M4`  : $$log(\lambda_i) =  log({aire_{i}}) + \mu_0 + \alpha_0 * cov_{2004}+\alpha_1 * cov_{2005} +\alpha_2 * cov_{2006} + \beta * prev_i $$
...
res4 <- M4$BUGSoutput$sims.matrix %>% 
  as.data.frame()
hist(res4$mu.0)
hist(res4$alpha_0)
hist(res4$alpha_1)
...
# Rétrotransformation
shells4 <- matrix(NA,ncol=nrow(gopher),nrow=nrow(res4))
names(res4)
for (i in 1:nrow(gopher)){
  shells4[,i] <-gopher$Area[i] * exp(res4$mu.0 + res4$alpha_0 * gopher$Cov_2004[i] + res4$alpha_1 * gopher$Cov_2005[i] + res4$alpha_2 * gopher$Cov_2006[i] + res4$b.prev)
}
hist(shells4)
mean(shells4)
...
Modèle avec année et site `M5` : $$log(\lambda_i) =  log({aire_{i}}) + \mu_0 + \alpha_0 * cov_{2004}+\alpha_1 * cov_{2005} +\alpha_2 * cov_{2006}+ \gamma_j\:\:avec\:\:j=1,...,10$$
...
# On récupère les paramètres et on regarde leur distribution
res5 <- M5$BUGSoutput$sims.matrix %>% 
  as.data.frame()
hist(res5$mu.0)
hist(res5$alpha_0)
hist(res5$alpha_1)
...
shells5 <- matrix(NA,ncol=nrow(gopher),nrow=nrow(res5))
for (i in 1:nrow(gopher)){
  shells5[,i] <-gopher$Area[i] * exp(res5$mu.0 + res5$alpha_0 * gopher$Cov_2004[i] + res5$alpha_1 * gopher$Cov_2005[i] + res5$alpha_2 * gopher$Cov_2006[i] +rnorm(1,mean=0, sd=res5$sd.s))
}
hist(shells5)
mean(shells5)
