...
aggregate(gopher$shells, list(gopher$Site), FUN=mean)
aggregate(gopher$prev, list(gopher$Site), FUN=mean)
```
...
Nous remqarquons également que certains sites ont de faibles prévalences (BS, Ord, FE) quelque soit l'année et d'autres des prévalences élevées (CF, GH, TE). 
```{r echo=FALSE}
...
```
En considérant l'histogramme, Ainsi, nous pourrons séparer les sites en deux catégories, ceux à faible ou forte prévalence. Cette variables sera donc traitée de deux manières : de manière continue et de manière discontinue avec deux catégories : 
